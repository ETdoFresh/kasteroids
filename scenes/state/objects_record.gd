...
func assign_ids():
    ...
    for i in range(array.size()):
        if array[i].id.value <= 0:
            result.array[i] = array[i].duplicate()
            result.array[i].id = array[i].id.init(next_id)
            next_id += 1
    return result
...
func set_cooldowns(delta: float):
...
func update_destroy_timers(delta: float):
...
func create_bounding_boxes():
    ...
    return map_only(has_bounding_box, create_bounding_box)
