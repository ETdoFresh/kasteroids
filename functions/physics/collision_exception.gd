...
static func add_collision_exception(object: Dictionary, other_object: Dictionary) -> Dictionary:
    object = object.duplicate()
    if not "collision_exceptions" in object:
        object["collision_exceptions"] = []
    object.collision_exceptions = LIST.append(object.collision_exceptions, other_object.id)
    return object
